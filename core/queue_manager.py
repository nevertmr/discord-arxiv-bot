...
class QueueManager:
    ...
    def is_duplicate(self, arxiv_id: str, published_date: str) -> bool:
        """중복 체크: completed와 pending 모두 확인"""
        # 1. completed 체크
        ...
        if completed_file.exists():
            try:
                ...
                if any(p['arxiv_id'] == arxiv_id for p in completed):
                    return True
            except Exception as e:
                self.logger.error(f'completed 중복 체크 실패: {e}')

        # 2. pending 체크
        try:
            for _f in self.pending_dir.glob(f'*_{arxiv_id}.json'):
                return True
        except Exception as e:
            self.logger.error(f'pending 중복 체크 실패: {e}')
    ...
    def restore_processing_to_pending(self):
        ...
        if self.processing_file.exists():
            try:
                with open(self.processing_file, encoding='utf-8') as f:
